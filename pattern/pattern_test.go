...
var regexpTests = []struct {
	...
	{
		pat: `[x/[:wrong:]]`, mode: EntireString | Filenames, want: `(?s)^\[x/\[:wrong:\]\]$`,
		mustMatch:    []string{"[x/[:wrong:]]"},
		mustNotMatch: []string{"/", "x"},
	},
	{
		pat: `[[.x.]/]`, mode: EntireString | Filenames, want: `(?s)^\[\[\.x\.\]/\]$`,
		mustMatch:    []string{"[[.x.]/]"},
		mustNotMatch: []string{"/", "x"},
	},
	{
		pat: `[x/[.x.]]`, mode: EntireString | Filenames, want: `(?s)^\[x/\[\.x\.\]\]$`,
	...
	{pat: `[a[:digit:]_]`, want: `(?s)[a[:digit:]_]`},
	{pat: `[[:digit:]a-z]`, want: `(?s)[[:digit:]a-z]`},
	{pat: `[[:alpha:][:digit:]_]`, want: `(?s)[[:alpha:][:digit:]_]`},
	{pat: `[!a[:space:]0-9]`, want: `(?s)[^a[:space:]0-9]`},
	{pat: `[![:space]]`, wantErr: `^charClass invalid$`},
